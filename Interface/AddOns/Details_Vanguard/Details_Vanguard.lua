...
Vanguard:SetPluginDescription ("在团本中显示每个坦克上的debuff，还显示预治疗和伤害以及你最后的命中.")
	...
	function Vanguard:OnDetailsEvent (event, ...)
		...
		elseif (event == "SHOW") then --> plugin shown, enabled
			...
			if (not Vanguard.db.first_run) then
				...
				local str = _detalhes.gump:CreateLabel (welcome, "欢迎使用坦克助手!\n\n\n- 绿-左栏代表坦克的预治疗加上吸收.\n\n- 红-右边尝试计算进攻伤害以及最后5秒内坦克遭受的总伤害.", nil, nil, "GameFontNormal")
				str:SetPoint (15, -15)
				str:SetWidth (375)
...
local build_options_panel = function()
	...
	local options_frame = Vanguard:CreatePluginOptionsFrame ("VanguardOptionsWindow", "坦克助手设置", 1)
	...
	elseif (event == "ADDON_LOADED") then
		...
		if (AddonName == "Details_Vanguard") then
			...
			if (_G._detalhes) then
				...
				local install, saveddata = _G._detalhes:InstallPlugin ("TANK", "坦克助手", "Interface\\Icons\\INV_Shield_77", Vanguard, "DETAILS_PLUGIN_VANGUARD", MINIMAL_DETAILS_VERSION_REQUIRED, "Details! Team", "v2.0", default_saved_table)
