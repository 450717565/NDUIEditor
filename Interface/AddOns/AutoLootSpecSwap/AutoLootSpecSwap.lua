...
if IsAddOnLoaded("Aurora") then
	local ttlTxt = f:CreateFontString(nil, "OVERLAY", nil)
	ttlTxt:SetFont("Fonts/MORPHEUS.ttf", 15, "OUTLINE")
	ttlTxt:SetText("AutoLootSpecSwap")
	ttlTxt:SetWidth(450)
	ttlTxt:SetHeight(64)
	ttlTxt:SetPoint("TOP", 0, 18)
else
...
saveButtonDesc:SetFont(STANDARD_TEXT_FONT, 12, "OUTLINE")
saveButtonDesc:SetText("首领：<没有选择>\n左键：切换 右键：清除")
saveButtonDesc:SetWidth(450)
saveButtonDesc:SetHeight(64)
saveButtonDesc:SetPoint("BOTTOM", 0, -48)
...
defaultButtonDesc:SetFont(STANDARD_TEXT_FONT, 12, "OUTLINE")
defaultButtonDesc:SetText("默认（拾取结束后切回默认）\n左键：保存 右键：清除")
...
journalSaveButton:SetScript("OnUpdate",function(self)
	if(self:IsVisible()) then
		...
		if(type(bossName) == "string") then
			...
			saveButtonDesc:SetText("首领："..bossName.."\n左键：切换 右键：清除")
...
loadframe:SetScript("OnEvent",function(self,event,addon)
	if(addon == "Blizzard_EncounterJournal") then
		...
		journalRestoreButton:SetWidth(70)
		journalRestoreButton:SetHeight(17)
		journalRestoreButton:SetText("设定专精")
		journalRestoreButton:SetParent(EncounterJournal)
		journalRestoreButton:ClearAllPoints()
		journalRestoreButton:SetPoint("RIGHT", EncounterJournalCloseButton, "LEFT", -2, 0)
...
if IsAddOnLoaded("Aurora") then
	...
	F.CreateBD(f)
	F.CreateSD(f)
	F.ReskinClose(fClose)
	F.Reskin(journalRestoreButton)
end
