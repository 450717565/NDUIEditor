...
local function EncounterJournal_InitLootFilter_Mine(self, level)
	...
	if (UIDROPDOWNMENU_MENU_VALUE == "range") then
		...
		makeSubInfo(info, ELP.LAST_RAID_NAME, 4, "range", level)
		makeSubInfo(info, ELP.LAST_RAID_NAME .. "和五人本", 5, "range", level)
...
EncounterJournal_OnFilterChanged_ELP = function()
	if db.range == 0 then
		return ELP.RunHooked("EncounterJournal_OnFilterChanged")
	else
		CloseDropDownMenus(1);
		ELP.UpdateItemList()
	end
...
ELP.initMenus = function()
	ELP.Hook("EncounterJournal_OnFilterChanged", EncounterJournal_OnFilterChanged_ELP)
