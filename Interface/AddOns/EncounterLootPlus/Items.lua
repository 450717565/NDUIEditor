...
function ELP.UpdateItemList()
	...
	local forceSlot = db.range > 0 and db.range ~= 4 and C_EncounterJournal.GetSlotFilter() == ELP.ALL_SLOT
	--if forceSlot then C_EncounterJournal.SetSlotFilter(ELP.RING_SLOT) end

	local range, lastRaid = db.range, nil
	if range == 4 or range == 5 then
		lastRaid = ELP.LAST_RAID_IDX
	...
	if forceSlot then C_EncounterJournal.SetSlotFilter(ELP.ALL_SLOT) end
...
function ELP.ScanStats(itemLink)
	...
	local stats = ELP.U1GetItemStats(itemLink, nil, nil, false, select(3, UnitClass("player")), GetSpecializationInfo(GetSpecialization() or 0))
