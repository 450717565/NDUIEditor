...
function NameplateSCT:DamageEvent(guid, spellID, amount, school, crit)
	...
	if (icon ~= "only") then
		-- truncate
		if (self.db.global.truncate and amount >= 1e8) then
			text = string.format("%.2f亿", amount / 1e8);
		elseif (self.db.global.truncate and amount >= 1e4) then
			text = string.format("%.1f万", amount / 1e4);
		else
