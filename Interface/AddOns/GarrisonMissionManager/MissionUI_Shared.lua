...
function addon_env.MissionList_ButtonsInit(blizzard_mission_list, frame_prefix)
   ...
   for idx = 1, #blizzard_buttons do
      ...
      if not gmm_frames[frame_prefix .. 'ExpirationText' .. idx] then
         ...
         expiration:SetWidth(200)
         expiration:SetHeight(1)
         expiration:SetPoint("RIGHT", blizzard_button, "RIGHT", -100, 0)
...
local mission_expiration_format_days  = "%s" .. "%d天%d时%d分"
local mission_expiration_format_hours = "%s" .. "%d时%d分"
local mission_expiration_format_minutes = "%s" .. "%d分"
      ...
      if index <= numMissions then
         ...
         if offerEndTime then
            ...
            if not xp_only_rewards then
               ...
               if days > 0 then
                  gmm_frames[frame_prefix .. 'ExpirationText' .. i]:SetFormattedText("剩余时间："..mission_expiration_format_days, color_code, days, hours, minutes)
               elseif hours > 0 then
                  gmm_frames[frame_prefix .. 'ExpirationText' .. i]:SetFormattedText("剩余时间："..mission_expiration_format_hours, color_code, hours, minutes)
			   else
				  gmm_frames[frame_prefix .. 'ExpirationText' .. i]:SetFormattedText("剩余时间："..mission_expiration_format_minutes, color_code, minutes)
               end
