Skada:AddLoadableModule("Damage", nil, function(Skada, L)
	...
	function modDetails:Update(win, set)
		...
		local damageStaggered = staggerabsorbed
				...
				staggerTaken.valuetext = Skada:FormatNumber(playerStagger.taken)..(" (%.1f%%)"):format(playerStagger.taken / damageStaggered * 100)
				...
				if playerStagger.purifyCount > 0 then
					...
					staggerPurified.valuetext = Skada:FormatNumber(playerStagger.purified)..(" (%.1f%%)"):format(playerStagger.purified / damageStaggered * 100)
		...
		if debug then
			...
			staggerPurified_s.valuetext = Skada:FormatNumber(playerStagger.purified_static)..(" (%.1f%%)"):format(playerStagger.purified_static / damageStaggered * 100)
			...
			staggerquicksip_s.valuetext = Skada:FormatNumber(playerStagger.purified_quicksip_static)..(" (%.1f%%)"):format(playerStagger.purified_quicksip_static / damageStaggered * 100)
		...
		if playerStagger.purified_quicksip > 0 then
			...
			staggerquicksip.label = "浅斟快饮"
			staggerquicksip.valuetext = Skada:FormatNumber(playerStagger.purified_quicksip)..(" (%.1f%%)"):format(playerStagger.purified_quicksip / damageStaggered * 100)
		...
		if other> 0 then
			...
			o.label = "其他"
			o.valuetext = Skada:FormatNumber(other)..(" (%.1f%%)"):format(other / damageStaggered * 100)
			o.value = other / damageStaggered
				...
				if setDuration > 0 and playerStagger.duration > 0 then
					...
					staggerDuration.valuetext = ("%.1f秒"):format(playerStagger.duration)
					...
					if playerStagger.freezeDuration > 2 then
							...
							freezeDuration.valuetext = ("%.1f秒"):format(playerStagger.freezeDuration)..(" (%.1f%%)"):format(playerStagger.freezeDuration / playerStagger.duration * 100)
