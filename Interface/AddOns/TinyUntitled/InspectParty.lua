...
local EnablePartyItemLevel = true
local ShowPartySpecialization = true
local SendPartyItemLevelToSelf = true
local SendPartyItemLevelToParty = false
...
LibEvent:attachEvent("GROUP_ROSTER_UPDATE", function(self)
	if not EnablePartyItemLevel then return end
...
local function PrintItemLevel(members)
	...
	if ShowPartySpecialization then
...
LibEvent:attachTrigger("PARTY_INSPECT_DONE, PARTY_INSPECT_TIMEOUT", function(self, members)
	if SendPartyItemLevelToSelf then
		PrintItemLevel(members)
	end
	if SendPartyItemLevelToParty then
