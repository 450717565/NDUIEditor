...
_detalhes.EncounterDetailsTempWindow = function (EncounterDetails)
	...
	local build_options_panel = function()
		...
		local options_frame = EncounterDetails:CreatePluginOptionsFrame ("EncounterDetailsOptionsWindow", "战斗细节设置", 2)
		...
		local on_show_menu = {
			{value = 1, label = "在团本中", onclick = set, desc = "仅在团本内显示图标."},
			{value = 2, label = "在小队中", onclick = set, desc = "仅在小队内显示图标."},
			{value = 3, label = "战斗结束", onclick = set, desc = "BOSS战斗后显示图标."},
			{value = 4, label = "总是", onclick = set, desc = "总是显示图标."},
			{value = 5, label = "自动", onclick = set, desc = "该插件决定何时需要显示图标."},
		}
...
--		/dump DETAILS_PLUGIN_ENCOUNTER_DETAILS.db.show_icon
	...
	function EncounterDetails:BuildDpsGraphic()
		...
		if (not graphicData or not combat.start_time or not combat.end_time) then
			EncounterDetails:Msg ("此分部没有图表数据.")
			return
	...
	function EncounterDetails:CreateGraphPanel()
		...
		--> build the phase panel
			...
			local damage_label = DetailsFrameWork:CreateLabel (phase_panel, "伤害:")
			...
			local heal_label = DetailsFrameWork:CreateLabel (phase_panel, "治疗:")
			...
			phase_panel.report_label = DetailsFrameWork:CreateLabel (phase_panel, "|cFFffb400左键|r: 报告伤害 |cFFffb400右键|r: 报告治疗")
			...
			local phase_on_enter = function (self)
				...
				if (combat) then
					...
					phase_panel.phase_label.text = "|cFFffb400片段|r: " .. self.phase
			...
			local phase_on_click = function (self, button)
				if (button == "LeftButton") then
					...
					tinsert (result, "Details!: "..(g.combat and g.combat.is_boss and g.combat.is_boss.name or "未知").. self.phase .. "片段的伤害 ".. ":")
				...
				elseif (button == "RightButton") then
					...
					tinsert (result, "Details!: "..(g.combat and g.combat.is_boss and g.combat.is_boss.name or "未知").. self.phase .. "片段的治疗 ".. ":")
		...
		--> chart frame implementations
			...
			DetailsFrameWork:NewLabel (EncounterDetails.Frame, EncounterDetails.Frame, nil, "phases_string", "片段:", "GameFontHighlightSmall")
	...
	--> Nome do Encontro
		DetailsFrameWork:NewLabel (headerFrame, headerFrame, nil, "boss_name", "未知战斗", "QuestFont_Large")
		BossFrame.boss_name = headerFrame.boss_name
		BossFrame.boss_name:SetPoint ("TOPLEFT", BossFrame, "TOPLEFT", 100, -46)
		
	--> Nome da Raid
		DetailsFrameWork:NewLabel (headerFrame, headerFrame, nil, "raid_name", "未知团本", "GameFontHighlightSmall")
	...
	--summary
	BossFrame.buttonSwitchNormal = _detalhes.gump:CreateButton (BossFrame, BossFrame.switch, BUTTON_WIDTH, BUTTON_HEIGHT, "摘要", "main")
	...
	--phases
	BossFrame.buttonSwitchPhases = _detalhes.gump:CreateButton (BossFrame, BossFrame.switch, BUTTON_WIDTH, BUTTON_HEIGHT, "片段", "phases")
	...
	--chart
	BossFrame.buttonSwitchGraphic = _detalhes.gump:CreateButton (BossFrame, BossFrame.switch, BUTTON_WIDTH, BUTTON_HEIGHT, "图表", "graph")
	...
	--emotes
	BossFrame.buttonSwitchBossEmotes = _detalhes.gump:CreateButton (BossFrame, BossFrame.switch, BUTTON_WIDTH, BUTTON_HEIGHT, "台词", "emotes")
...
--> ~emotes
	...
	local row_on_mouse_up = function (self)
		...
		local reportFunc = function()
			...
			EncounterDetails:SendReportLines ({"Details! 战斗台词在" .. time, "\"" .. text .. "\""})
		end
	...
	local emotes_segment_label = DetailsFrameWork:CreateLabel (BossFrame, "段:", 11, nil, "GameFontHighlightSmall")
	...
	local build_emote_segments = function()
		...
		for index, segment in ipairs (EncounterDetails.charsaved.emotes) do
			tinsert (t, {label = "#" .. index .. " "  .. (segment.boss or "未知"), value = index, icon = segment_icon, texcoord = segment_icon_coord, onclick = on_emote_Segment_select, iconcolor = segment_icon_color})
		end
		return t
	end
	...
	--search box
	local emotes_search_label = DetailsFrameWork:CreateLabel (BossFrame, "查找:", 11, nil, "GameFontHighlightSmall")
	...
	local report_emote_button = DetailsFrameWork:NewButton (BossFrame, nil, "$parentReportEmoteButton", "ReportEmoteButton", 180, 20, function()
		local reportFunc = function (IsCurrent, IsReverse, AmtLines)
			...
			if (segment) then
				EncounterDetails.report_lines = {"Details!:".. segment.boss .. " 的台词 " }
				local added = 0
	...
	local emote_report_label = DetailsFrameWork:NewLabel (search.widget, search.widget, nil, "report_click", "|cFFffb400左键点击|r: 报告行")
...
--> auras weakauras
	...
	local EnemyActorSpells_label = DetailsFrameWork:CreateLabel (BossFrame, "敌军部队:", 11, nil, "GameFontHighlightSmall")
	...
	local info_onenter = function (self)
		...
		if (info) then
			...
			for token, _ in pairs (info.token) do
				GameCooltip:AddLine ("事件:", token, 1, nil, "white")
			end
			
			GameCooltip:AddLine ("来源:", info.source, 1, nil, "white")
			GameCooltip:AddLine ("类型:", EncounterDetails:GetSpellSchoolFormatedName (info.school), 1, nil, "white")
			
			if (info.type) then
				GameCooltip:AddLine ("光环类型:", info.type, 1, nil, "white")
			end
	...
	for i = 1, CONST_MAX_AURA_LINES do
		...
		spellinfotext:SetText ("信息")
		spellinfo:SetPoint ("left", spellname.widget, "right", 4, 0)
		
		local create_aura = DetailsFrameWork:NewButton (anchor_frame, nil, "$parentCreateAuraButton", "AuraButton", 135, 18, create_aura_func, nil, nil, nil, "创建光环")
	...
	--
	local label_dbm_bars = DetailsFrameWork:CreateLabel (BossFrame, "创建大计时器警告 (DBM):", 11, nil, "GameFontHighlightSmall")
	label_dbm_bars:SetPoint ("topleft", BossFrame, "topleft", 10, -200)
	
	local label_bw_bars = DetailsFrameWork:CreateLabel (BossFrame, "创建大计时器警告 (BW):", 11, nil, "GameFontHighlightSmall")
	...
	local on_select_dbm_bar = function (_, _, value)
		...
		EncounterDetails:OpenAuraPanel (timer_id, spellname, spellicon, encounterid, DETAILS_WA_TRIGGER_DBM_TIMER, DETAILS_WA_AURATYPE_TEXT, {dbm_timer_id = timer_id, spellid = spellid, text = "下一个 " .. spellname .. " 在", text_size = 72, icon = spellicon})
	end
	
	local on_select_bw_bar = function (_, _, value)
		local timer_id, spellname, spellicon, encounterid = unpack (value)
		EncounterDetails:OpenAuraPanel (timer_id, spellname, spellicon, encounterid, DETAILS_WA_TRIGGER_BW_TIMER, DETAILS_WA_AURATYPE_TEXT, {bw_timer_id = timer_id, text = "下一个 " .. spellname .. " 在", text_size = 72, icon = spellicon})
	end
	...
	dropdown_dbm_bars.tooltip = "创建一个weakaura来显示一个BOSS即将释放技能的CD"
	...
	dropdown_bw_bars.tooltip = "创建一个weakaura来显示一个BOSS即将释放技能的CD"
...
local PhaseSelectLabel = _detalhes.gump:CreateLabel (PhaseFrame, "选择片段:", 12, "orange")
local DamageLabel = _detalhes.gump:CreateLabel (PhaseFrame, "伤害")
local HealLabel = _detalhes.gump:CreateLabel (PhaseFrame, "治疗")
local PhaseTimersLabel = _detalhes.gump:CreateLabel (PhaseFrame, "每个片段花费时间")
...
local report_damage = function (IsCurrent, IsReverse, AmtLines)
	...
	tinsert (result, "Details!: " .. (PhaseFrame.CurrentSegment and PhaseFrame.CurrentSegment.is_boss and PhaseFrame.CurrentSegment.is_boss.name or "未知") .. PhaseFrame.LastPhaseSelected .. " 片段的伤害 " ..  ":")
...
local Report_DamageButton = _detalhes.gump:CreateButton (PhaseFrame, report_damage, 16, 16, "报告")
...
local report_healing = function()
	...
	tinsert (result, "Details!: " .. (PhaseFrame.CurrentSegment and PhaseFrame.CurrentSegment.is_boss and PhaseFrame.CurrentSegment.is_boss.name or "未知") .. PhaseFrame.LastPhaseSelected .. " 片段的治疗 " ..  ":")
...
local Report_HealingButton = _detalhes.gump:CreateButton (PhaseFrame, report_healing, 16, 16, "报告")
...
function PhaseFrame:UpdatePhaseBars()
	...
	for index, phase in ipairs (timers) do
		...
		PhaseFrame.PhasesBars [i].name:SetText ("|cFFC0C0C0片段:|r |cFFFFFFFF" .. phase)
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	--> Selecionar o segmento
		...
		local segmentos_string = DetailsFrameWork:NewLabel (frame, nil, nil, "segmentosString", "段:", "GameFontNormal", 12)
		...
		--> options button
		local options = DetailsFrameWork:NewButton (frame, nil, "$parentOptionsButton", "OptionsButton", 120, 20, EncounterDetails.OpenOptionsPanel, nil, nil, nil, "设置")
	...
	-- ~containers ~damagetaken
		...
		DetailsFrameWork:NewLabel (dano_recebido_bg, dano_recebido_bg, nil, "damagetaken_title", "每个玩家承受伤害", "GameFontHighlight") --localize-me
	...
	--~ability ~damage taken by spell
		...
		DetailsFrameWork:NewLabel (habilidades_inimigas_bg, habilidades_inimigas_bg, nil, "habilidades_title", "承受伤害的法术", "GameFontHighlight") --localize-me
	...
	-- ~adds �dds
		...
		local adds_total_string = DetailsFrameWork:CreateLabel (container_adds_window, "承受伤害")
		...
		DetailsFrameWork:NewLabel (container_adds_window, container_adds_window, nil, "titulo", "受到的敌人伤害", "GameFontHighlight")
	...
	-- ~interrupt
		...
		local interrupt_total_string = DetailsFrameWork:CreateLabel (container_interrupt_window, "打断 / 施法")
	...
	-- ~dispel
		...
		local dispell_total_string = DetailsFrameWork:CreateLabel (container_dispell_window, "总计驱散")
	...
	-- ~mortes ~deaths ~dead
		...
		local dead_total_string = DetailsFrameWork:CreateLabel (container_dead_window, "死亡时间")
