...
if cfg.Gold == true then
	...
	local Earned, Spent, OldMoney = 0, 0, 0
	...
	local function formatTextMoney(money)
		...
		local money = money * .0001
		if money <= 1e5 then
			return ("%.0f|cffffd700金|r"):format(money)
		else
			return ("%.2f|cffffd700万金|r"):format(money / 1e4)
		end
	end
	...
	Stat:SetScript("OnEnter", function(self)
		...
		GameTooltip:AddDoubleLine(ns.infoL["Earned"], GetMoneyString(Earned), 1,1,1, 1,1,1)
		GameTooltip:AddDoubleLine(ns.infoL["Spent"], GetMoneyString(Spent), 1,1,1, 1,1,1)
		if Earned < Spent then
			GameTooltip:AddDoubleLine(ns.infoL["Deficit"], GetMoneyString(Spent-Earned), 1,0,0, 1,1,1)
		elseif Earned > Spent then
			GameTooltip:AddDoubleLine(ns.infoL["Profit"], GetMoneyString(Earned-Spent), 0,1,0, 1,1,1)
		end
		GameTooltip:AddLine(" ")
