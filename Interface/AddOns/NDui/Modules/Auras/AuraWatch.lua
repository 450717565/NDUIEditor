...
local function ConvertTable()
	...
	InsertData(IntCD.List, myTable[11])
end
...
local function BuildICON(iconSize)
	...
	Frame.Count = B.CreateFS(parentFrame, iconSize*.55, "", false, "BOTTOMRIGHT", 3, -3)
...
local function BuildBAR(barWidth, iconSize)
	...
	Frame.Count = B.CreateFS(Frame, 14, "", false, "BOTTOMRIGHT", 1, -1)
...
local function UpdateCDFrame(index, name, icon, start, duration, bool, type, id, charges)
	...
	if Frame.Statusbar then
		...
		Frame:SetScript("OnUpdate", function(self, elapsed)
			...
			elseif self.Timer > 0 and self.Timer < 60 then
