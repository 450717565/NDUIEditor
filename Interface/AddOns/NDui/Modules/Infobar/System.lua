...
info.onUpdate = function(self, elapsed)
	...
	if self.timer > 1 then
		...
		self.text:SetText(colorFps(fps).."|rFps"..DB.MyColor.." / |r"..colorLatency(latency).."|rMs")
	...
	if GetCVar("scriptProfile") == "1" then
		...
		for i = 1, maxAddOns do
			...
			local color = percent <= 1 and {0, 1} -- 0 - 1
				...
				or {1, .1} -- 50 +
			GameTooltip:AddDoubleLine(usageTable[i][1], format("%.1f %%", percent), 1, 1, 1, color[1], color[2], 0)
		end
		...
		if not IsShiftKeyDown() then
			...
			if #usageTable > C.Infobar.MaxAddOns then
				...
				GameTooltip:AddDoubleLine(format("%d %s (%s)", numHidden, L["Hidden"], L["Shift"]), format("%.1f %%", hiddenUsage/totalUsage*100), .6,.8,1, .6,.8,1)
			end
		end
		GameTooltip:AddLine(" ")
	end
