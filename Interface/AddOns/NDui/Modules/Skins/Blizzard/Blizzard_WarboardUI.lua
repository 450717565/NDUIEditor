...
C.themes["Blizzard_WarboardUI"] = function()
	...
	local function reskinFrame(self)
		if not self.styled then
			...
			self.CloseButton.SetPoint = B.Dummy
		...
		for i = 1, self:GetNumOptions() do
			...
			if not Options.styled then
				...
				B.ReskinButton(Options.OptionButtonsContainer.OptionButton1)
				B.ReskinButton(Options.OptionButtonsContainer.OptionButton2)
