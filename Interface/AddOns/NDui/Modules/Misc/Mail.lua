...
function M:MailItem_OnClick()
	mailItemIndex = 7 * (InboxFrame.pageNum - 1) + tonumber(sub(self:GetDebugName(), 9, 9))
...
function M:MailItem_AddDelete(i)
	...
	bu:SetPoint("BOTTOMRIGHT", self:GetParent(), "BOTTOMRIGHT", -12, 5)
	bu:SetSize(16, 16)
...
function M:MailBox()
	...
	button1 = M:CreatButton(InboxFrame, L["Collect All"], 80, 28, "TOPRIGHT", MailFrame, "TOP", -30, -30)
	button1:RegisterEvent("MAIL_CLOSED")
	button1:SetScript("OnClick", M.MailBox_OpenAll)
	button1:SetScript("OnEvent", M.MailBox_OnEvent)

	button2 = M:CreatButton(InboxFrame, L["Collect Gold"], 80, 28, "TOPLEFT", MailFrame, "TOP", 30, -30)
	...
	button3 = M:CreatButton(OpenMailFrame, L["Collect Letters"], 80, 22, "RIGHT", OpenMailReplyButton, "LEFT", -1, 0)
