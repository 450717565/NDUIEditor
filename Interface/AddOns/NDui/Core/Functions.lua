...
function B.FormatTime(s)
	if s >= day then
		return format("%d"..DB.MyColor..L["Days"], ceil(s/day)), s%day
	elseif s >= hour then
		return format("%d"..DB.MyColor..L["Hours"], ceil(s/hour)), s%hour
	elseif s >= minute then
		return format("%d"..DB.MyColor..L["Minutes"], ceil(s/minute)), s%minute
	elseif s > 10 then
