...
local function YesTutor()
	...
	B.CreateGF(ll, 80, .5, "Horizontal", cr, cg, cb, 0, .7)
	ll:SetFrameStrata("HIGH")
	local lr = CreateFrame("Frame", nil, tutor)
	lr:SetPoint("TOP", 41, -32)
	B.CreateGF(lr, 80, .5, "Horizontal", cr, cg, cb, .7, 0)
...
local function HelloWorld()
	...
	B.CreateGF(ll, 100, .5, "Horizontal", cr, cg, cb, 0, .7)
	ll:SetFrameStrata("HIGH")
	local lr = CreateFrame("Frame", nil, welcome)
	lr:SetPoint("TOP", 51, -35)
	B.CreateGF(lr, 100, .5, "Horizontal", cr, cg, cb, .7, 0)
	lr:SetFrameStrata("HIGH")
