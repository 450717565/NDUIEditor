...
L["Action*"] = "行为*"
L["Action Intro"] = "|n为团队框架设置按键行为。|n|n输入target，设置目标；|n|n输入focus，设置焦点；|n|n输入follow，设置跟随；|n|n输入数字，设置施法的法术ID；|n|n你也可以直接输入宏。|n对于存在多行内容的宏命令，用||n来进行换行。"
L["Key*"] = "按键*"
L["Key Intro"] = "|n为该法术绑定一个鼠标按键。|n|n为防止冲突，不建议设置单独的左键或者中键。"
L["Modified Key"] = "功能键"
L["ModKey Intro"] = "|n为该法术设置组合键。|n|n为防止冲突，不建议设置组合键 SHIFT+左键。"
L["Enable ClickSets"] = "启用团队框架快速施法"
