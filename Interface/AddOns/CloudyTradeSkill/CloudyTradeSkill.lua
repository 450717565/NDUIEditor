...
local function InitDB()
	...
	if IsAddOnLoaded('AuroraClassic') then
		skinUI = 'AuroraClassic'
		F, C = unpack(AuroraClassic)
	elseif IsAddOnLoaded('ElvUI') then
		skinUI = 'ElvUI'
		F = unpack(ElvUI):GetModule('Skins')
	end
end
	...
	local function addTab(id, index, isSub)
		...
		if skinUI and not tab.skinned then
			...
			if (skinUI == 'AuroraClassic') then
				tab:SetSize(34, 34)
				checkedTexture = C.media.checked

				local bg = F.CreateBDFrame(tab:GetNormalTexture(), 0)
		...
		for i = 1, numTabs do
			...
			if tab then
				if CTradeSkillDB['Tabs'][tab.id] == true then
					tab:SetPoint('TOPLEFT', TradeSkillFrame, 'TOPRIGHT', skinUI and 3 or 0, (-44 * index) + (-40 * tab.isSub))
					tab:Show()
					index = index + 1
				else
					tab:Hide()
...
local function injectDruidButtons()
	...
	local function injectMacro(button, text)
		...
		if (skinUI == 'AuroraClassic') then
			F.ReskinButton(macro)
		elseif (skinUI == 'ElvUI') then
			F:HandleButton(macro, true)
		end
...
local function injectVellumButton()
	...
	if (skinUI == 'AuroraClassic') then
		F.ReskinButton(vellum)
	elseif (skinUI == 'ElvUI') then
		F:HandleButton(vellum, true)
	end
