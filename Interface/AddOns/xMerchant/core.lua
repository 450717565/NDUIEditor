...
local function GetError(link, isRecipe)
	...
	for i=2, tooltip:NumLines() do
		...
		XMERCHANT_LOG_DEBUG("[GetError]  Line: "..i.."   TooltipTextLeft: "..(_G["xMerchantTooltipTextLeft"..i]:GetText() or ""))
		XMERCHANT_LOG_DEBUG("[GetError]  Line: "..i.."   TooltipTextRight: "..(_G["xMerchantTooltipTextRight"..i]:GetText() or ""))
...
local function CurrencyUpdate()
	...
	for i=1, limit do
		...
		if not isHeader then
			XMERCHANT_LOG_DEBUG("[CurrencyUpdate]  ".."  name: "..(name or "nil").."  count: "..(count or "nil").."  maxi: "..(maximum or "nil").."  itemID: "..(itemID or "nil"))
		end
...
local function AltCurrencyFrame_Update(item, texture, cost, itemID, currencyName)
	...
	item:SetSize(item.count:GetWidth() + iconWidth + 4, item.count:GetHeight() + 4)
end
	...
	for i=1, 10, 1 do
		...
		if offset <= numMerchantItems then
			...
			if link then
				...
				local alpha = 0.3
				if searching == ""
				or (searching == SEARCH:lower())
				or (name:lower():match(searching))
				or (itemRarity and tostring(itemRarity):lower():match(searching))
				or (_G["ITEM_QUALITY"..tostring(itemRarity).."_DESC"]:lower():match(searching))
...
for i=1, 10, 1 do
	...
	button:SetSize(frame:GetWidth(), 29.4)
	if i == 1 then
	...
	local itemname_fontsize = getCurrentDB().itemname_fontsize or 16
	local iteminfo_fontsize = getCurrentDB().iteminfo_fontsize or 14
...
for _, frame in next, { MerchantNextPageButton, MerchantPrevPageButton, MerchantPageText, MerchantRepairText } do
	frame:Hide()
