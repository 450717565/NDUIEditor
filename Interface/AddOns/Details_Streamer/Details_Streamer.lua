...
local function CreatePluginFrames()
	...
	titlebar.text:SetText ("Details! 主播: 动作追踪器")
	titlebar:SetScript ("OnEnter", function (self) 
		GameTooltip:SetOwner (self)
		GameTooltip:SetOwner (self, "ANCHOR_TOPLEFT")
		GameTooltip:AddLine ("|cFFFF7700左键|r: 打开设置\n|cFFFF7700右键|r: 锁定框体\n|cFFFF7700斜线命令|r: /streamer")
		GameTooltip:Show()
	end)
...
function StreamOverlay.OpenOptionsPanel (from_options_panel)
	...
	if (not StreamOverlayOptionsPanel) then
		...
		local options_frame = StreamOverlay:CreatePluginOptionsFrame ("StreamOverlayOptionsPanel", "Details! 主播: 动作追踪器", 1)
		...
		local strataTable = {
			{value = "BACKGROUND", label = "背景", onclick = set_window_strata, icon = [[Interface\Buttons\UI-MicroStream-Green]], iconcolor = {0, .5, 0, .8}, texcoord = nil},
			{value = "LOW", label = "低", onclick = set_window_strata, icon = [[Interface\Buttons\UI-MicroStream-Green]] , texcoord = nil},
			{value = "MEDIUM", label = "中", onclick = set_window_strata, icon = [[Interface\Buttons\UI-MicroStream-Yellow]] , texcoord = nil},
			{value = "HIGH", label = "高", onclick = set_window_strata, icon = [[Interface\Buttons\UI-MicroStream-Yellow]] , iconcolor = {1, .7, 0, 1}, texcoord = nil},
			{value = "DIALOG", label = "顶端", onclick = set_window_strata, icon = [[Interface\Buttons\UI-MicroStream-Red]] , iconcolor = {1, 0, 0, 1},  texcoord = nil},
		}
		--
		...
		local options = {
			...
			
			{
				...
				desc = "框架在界面中放置多高，高值使其显示在背包，天赋框架等上方.",
				name = "窗体层级"
			},
		...
		local label_profile = Details.gump:CreateLabel (options_frame, "配置" .. ": ", Details.gump:GetTemplate ("font", "OPTIONS_FONT_TEMPLATE"))
		...
		if (not Details_StreamerDB.profiles [pname]) then
			...
			options_frame.NewProfileButton = Details.gump:CreateButton (options_frame, add_profile, 60, 18, "新建配置", _, _, _, _, _, _, Details.gump:GetTemplate ("dropdown", "OPTIONS_DROPDOWN_TEMPLATE"), Details.gump:GetTemplate ("font", "OPTIONS_FONT_TEMPLATE"))
	...
	if (event == "ADDON_LOADED") then
		...
		if (AddonName == "Details_Streamer") then
			...
			if (_G._detalhes) then
				...
				--> Install
				local install, saveddata = _G._detalhes:InstallPlugin ("TOOLBAR", "主播", [[Interface\MINIMAP\MOVIERECORDINGICON]], StreamOverlay, "DETAILS_PLUGIN_STREAM_OVERLAY", MINIMAL_DETAILS_VERSION_REQUIRED, "Details! Team", StreamOverlay.CurrentVersion, default_options_table)
				...
				StreamOverlay:SetPluginDescription ("实时显示你正在释放的法术.\n\n观众现在可以跟随你正在做的事情，你正在释放的法术，学习你的循环.\n\n如果你在竞技场，还告诉观众目标是谁及其关于战斗或角色的职业/天赋.\n\n当你死了，面板上显示死亡日志.")
	...
	if LDB then
		local minimapIcon = LDB:NewDataObject ("DetailsStreamer", {
			...
			OnTooltipShow = function (tooltip)
				tooltip:AddLine ("Details!: 主播", 1, 1, 1)
				tooltip:AddLine ("|cFFFF7700左键|r: 打开设置.")
				tooltip:AddLine ("|cFFFF7700右键|r: 隐藏这个图标.")
			end,
		})
