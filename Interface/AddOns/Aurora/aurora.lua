...
Skin:SetScript("OnEvent", function(self, event, addon)
	...
	if addon == "Aurora" then
		...
		for _, button in pairs(professions) do
			...
			_G[button.."StatusBarLeft"]:SetAlpha(0)
			_G[button.."StatusBarRight"]:SetAlpha(0)
			_G[button.."StatusBarBGLeft"]:Hide()
			_G[button.."StatusBarBGMiddle"]:Hide()
			_G[button.."StatusBarBGRight"]:Hide()

			F.ReskinStatusBar(bu.statusBar, true)
		end
		...
		for i = 1, 6 do
			for j = 1, 3 do
				...
				local tab = _G["FriendsTabHeaderTab"..j]
		...
		ChatConfigFrameDefaultButton:SetWidth(100)
