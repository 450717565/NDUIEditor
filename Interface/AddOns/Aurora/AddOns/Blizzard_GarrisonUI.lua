...
C.themes["Blizzard_GarrisonUI"] = function()
	...
	do
		...
		local pframe = FollowerTab.PortraitFrame
		F.ReskinGarrisonPortrait(pframe)
	end
	...
	F.CreateSD(MissionPage.RewardsFrame)
	...
	local function onUpdateData(self)
		...
		for i = 1, #buttons do
			...
			if not button.restyled then
				...
				button.BusyFrame:SetAllPoints(button)
	...
	local function onShowFollower(self, followerId)
		...
		for i = 1, #allyspell do
			if not allyspell.styled then
				local spell = allyspell[i].iconTexture
				spell:SetTexCoord(.08, .92, .08, .92)
				F.CreateBDFrame(spell)
